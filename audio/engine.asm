...
.ch3_check_vol_override
	bit NOTE_VOLUME_OVERRIDE, [hl]
	jr z, .ch3_check_env_override
	call .load_wave_volume
...
.pluck
	bc_offset CHANNEL_FLAGS1
	bit SOUND_OCTAVE_PLUCK, [hl]
	jr nz, .set_pluck
	bc_offset CHANNEL_FLAGS3
	bit SOUND_FIFTHS_PLUCK, [hl]
	jr z, .duty_seq
.set_pluck
	bc_offset CHANNEL_FLAGS3
	set SOUND_PLUCK_HANDLING, [hl]

.duty_seq
	bc_offset CHANNEL_FLAGS2
	bit SOUND_DUTY_SEQUENCE, [hl]
	ret z
	bc_offset CHANNEL_DUTY_SEQUENCE_OFFSET
	xor a
	ld [hl], a
	ret
...
.pluck
	...
	dec [hl]
	ld d, [hl]
	; preserve main octave
	bc_offset CHANNEL_BACKUP_OCTAVE
	ld [hl], a
	; get pitch
...
.fifths
	...
	bit SOUND_PLUCK_HANDLING, [hl]
	jr z, .fifths_off
	; save base pitch / modify pitch
	bc_offset CHANNEL_PITCH
	ld e, [hl]
	push de
	; get designated spot in generic audio RAM
	ld a, [wCurChannel]
...
.fifths_off
	...
	ld hl, wSavedPitch
	add hl, de
	ld a, [hl] ; retrieve base pitch
	; did we already correct?
	bc_offset CHANNEL_PITCH
	cp [hl]
	jr z, .duty_seq
	; save to channel RAM
...
.duty_seq
	...
	bc_offset CHANNEL_NOTE_FLAGS
	set NOTE_DUTY_OVERRIDE, [hl]
	bc_offset CHANNEL_DUTY_SEQUENCE
	ld a, [hli] ; id
	ld e, a
	ld a, [hl]  ; offset
	ld d, 0
...
;	$e7 $f4-ff can trigger the rest pitch due to a pitch overflow
	bc_offset CHANNEL_FLAGS2
	bit SOUND_RELATIVE_PITCH, [hl]
	jr z, .pitch_offset
...
; $ff is reached in music data
	...
	jr nz, .ok
.chan_5to8
; see if restoring music channel output is possible
	; are we even running SFX?
	cp CHAN5
	jr c, .dont_restore
	; load the music equivalent channel
	push bc ; save SFX channel
	maskbits NUM_MUSIC_CHANS
	call LoadChannelPointer
	call RestoreMusicChannel
	pop bc
.dont_restore:
	bc_offset CHANNEL_FLAGS1
	bit SOUND_CRY, [hl]
...
; input: hl - sequence pointer selection
; output: a - envelope
	; get pointer
	add hl, de
	add hl, de
...
; entries correspond to audio constants (see macros/scripts/audio.asm)
	...
	dw Music_OctavePluckOn         ; octave pluck on
	dw Music_AllPlucksOff          ; plucks off
	dw Music_DutyCycleSequence     ; duty cycle sequence
	dw Music_FifthsPluckOn         ; fifths pluck on
	dw Music_Tie                   ; note tie
...
; params: 1
	bc_offset CHANNEL_FLAGS2
	set SOUND_ENV_SEQUENCE, [hl]
	call GetMusicByte
	bc_offset CHANNEL_ENV_SEQUENCE
	ld [hl], a
	ret

Music_DutyCycleSequence:
	bc_offset CHANNEL_FLAGS2
	set SOUND_DUTY_SEQUENCE, [hl]
	call GetMusicByte
	bc_offset CHANNEL_DUTY_SEQUENCE
	ld [hl], a
	ret
...
Music_OctavePluckOn:
	bc_offset CHANNEL_FLAGS1
	set SOUND_OCTAVE_PLUCK, [hl]
	ret

Music_FifthsPluckOn:
	bc_offset CHANNEL_FLAGS3
	set SOUND_FIFTHS_PLUCK, [hl]
	ret

Music_AllPlucksOff:
	bc_offset CHANNEL_FLAGS1
	res SOUND_OCTAVE_PLUCK, [hl]
	bc_offset CHANNEL_FLAGS3
	res SOUND_FIFTHS_PLUCK, [hl]
	ret
...
; reset sweep
	xor a
	ldh [rNR10], a
	ld hl, wChannel5NoteFlags
	res NOTE_PITCH_SWEEP, [hl]
...
RestoreMusicChannel:
; input: bc
; output: CHANNEL_NOTE_FLAGS.NOTE_NOISE_SAMPLING
	; is there stacato?
	bc_offset CHANNEL_FLAGS2
	bit SOUND_STACCATO, [hl]
	jr z, .no_staccato
	; yes
	; is the staccato counter still active?
	bc_offset CHANNEL_STACCATO_COUNTER
	ld a, [hl]
	and a
	jr z, .skip_restore
.no_staccato
	...
	; yes
.restore
	; see if we were playing a rest
	call LoadPreviousMusicByte
	jr z, .skip_restore
	; we reach here if the note is:
